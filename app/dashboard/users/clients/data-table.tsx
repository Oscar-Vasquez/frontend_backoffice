  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-4">
        <Avatar>
          <AvatarImage 
            src={processPhotoUrl(row.original.image)} 
            alt={row.getValue("name") || "user"}
            onError={(e) => {
              console.log('Error loading user image:', row.original.image);
              e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(row.getValue("name") || "User")}&background=random&color=fff&size=128&bold=true`;
            }}
          />
